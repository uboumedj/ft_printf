...
/*   Updated: 2017/12/20 07:19:56 by uboumedj         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
size_t			do_bnb(va_list *vlist, int mod, t_printf *handler)
{
	...
	return (print_bnb(&res, handler));
}

static int		bnb_length(unsigned long long int nb, t_printf *handler)
{
	int		ilen;
	int		res;
...
size_t      print_bnb(unsigned long long int res, t_printf *handler)
{
  int		ilen;
	...
	if (!handler)
		return (0);
	ilen = ft_nbrlenbase(res, 2);
	len = bnb_length(res, handler);
